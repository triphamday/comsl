...
# language comsl lookup by name, with a few language aliases
TO_LANGUAGE_CODE = {
...
class Tokenizer:
    ...
    task: Optional[str]
    predict_timestamps: Optional[bool]
    ...
    @property
    def all_language_dict(self) -> Tuple[int]:
        result = {}
        for token, token_id in zip(
            self.tokenizer.additional_special_tokens,
            self.tokenizer.additional_special_tokens_ids,
...
def get_tokenizer(
    ...
    *,
    ...
    return Tokenizer(tokenizer=tokenizer, language=language, task=task, predict_timestamps=False, sot_sequence=tuple(sot_sequence))
